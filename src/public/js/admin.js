(function () {
  ...
  function applyAccessibility(on) {
    ...
    if (!indicator) {
      ...
      indicator.style.cssText =
        'position:fixed;bottom:8px;right:8px;background:var(--accent);color:#fff;font-size:0.7rem;padding:0.2rem 0.6rem;border-radius:6px;z-index:9999;';
  ...
  function renderSkeleton(rows, cols) {
    ...
    for (var r = 0; r < rows; r++) {
      ...
      for (var c = 0; c < cols; c++) {
        h +=
          '<div class="skel-cell skel" style="flex:' + (c === 0 ? 1 : 2) + ';height:1rem;"></div>';
      }
      h += '</div>';
    }
    return h;
  ...
  function showSkeleton(id, rows, cols) {
    ...
    el.innerHTML =
      '<div class="admin-card-body">' + renderSkeleton(rows || 5, cols || 5) + '</div>';
  }
  ...
  function emptyState(icon, text) {
    return (
      '<div class="empty-state"><i class="fas fa-' + icon + '"></i><p>' + esc(text) + '</p></div>'
    );
  }
  ...
  function openConfirm(title, desc, cb, opts) {
    ...
    var extra = byId('confirmExtra'),
      input = byId('confirmInput');
    if (opts.requireTyping) {
      extra.classList.remove('hidden');
      byId('confirmTypeWord').textContent = opts.requireTyping;
      input.value = '';
    ...
    _confirmCallback = cb;
    _confirmStep2 = opts.requireTyping || null;
    showEl('confirmModal');
    input.focus();
  }
  byId('confirmCancel').addEventListener('click', function () {
    hideEl('confirmModal');
  ...
  byId('confirmOk').addEventListener('click', function () {
    if (_confirmStep2) {
      var val = byId('confirmInput').value.trim();
  ...
  byId('confirmInput').addEventListener('keydown', function (e) {
    if (e.key === 'Enter') byId('confirmOk').click();
  });

  // ---- Ban modal ----
  byId('banReasonSelect').addEventListener('change', function () {
    var txt = byId('banReasonText');
    this.value === 'Other' ? txt.classList.remove('hidden') : txt.classList.add('hidden');
  });
  byId('banCancel').addEventListener('click', function () {
    hideEl('banModal');
    _pendingBanUserId = null;
  });
  byId('banConfirm').addEventListener('click', function () {
    var uid = _pendingBanUserId;
    if (!uid) return;
    ...
    if (reason === 'Other') {
      var custom = byId('banReasonText').value.trim();
      if (!custom) {
  ...
  function openBanModal(userId, label) {
    ...
    byId('banUserLabel').textContent = 'Ban ' + label + ' (ID: ' + userId + ')';
    byId('banDuration').value = '1';
    byId('banReasonSelect').value = '';
    byId('banReasonText').value = '';
    byId('banReasonText').classList.add('hidden');
    showEl('banModal');
  }
    ...
    ids.forEach(function (id) {
      authFetch('/auth/admin/ban-with-reason', {
        ...
        .catch(function (err) {
          ...
          if (done === ids.length) {
            alert('Ban failed for some users: ' + err.message);
          }
        });
    });
  }
  ...
  function setBreadcrumb(items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (i === items.length - 1)
        h += '<span class="bc-current">' + esc(items[i].label) + '</span>';
  ...
  byId('btnCmdPalette').addEventListener('click', function () {
    openCmdPalette();
  });
  ...
  byId('cmdOverlay').addEventListener('click', function (e) {
  ...
  byId('cmdInput').addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Enter') {
      var items = byId('cmdResults').querySelectorAll('.cmd-item');
      if (items.length > 0) items[0].click();
    }
  });
  function performGlobalSearch(q) {
    authFetch('/auth/admin/global-search?q=' + encodeURIComponent(q))
      .then(function (data) {
        var el = byId('cmdResults');
        if (!data || (!data.users.length && !data.posts.length && !data.reports.length)) {
          el.innerHTML =
            '<div class="cmd-empty"><i class="fas fa-search me-2"></i>No results for "' +
            esc(q) +
            '"</div>';
          return;
        }
        ...
        if (data.users.length) {
          h +=
            '<div class="cmd-group"><div class="cmd-group-label"><i class="fas fa-users me-1"></i>Users</div>';
          data.users.forEach(function (u) {
            h +=
              '<div class="cmd-item" data-type="user" data-id="' +
              u.id +
              '"><span class="cmd-type">User</span><strong>' +
        ...
        if (data.posts.length) {
          h +=
            '<div class="cmd-group"><div class="cmd-group-label"><i class="fas fa-newspaper me-1"></i>Posts</div>';
          data.posts.forEach(function (p) {
            h +=
              '<div class="cmd-item" data-type="post" data-id="' +
              p.id +
              '"><span class="cmd-type">Post</span><strong>' +
              esc(p.title || 'Untitled') +
              '</strong><span class="text-muted small"> by ' +
              esc(p.author_name) +
              '</span></div>';
          });
          h += '</div>';
        ...
        if (data.reports.length) {
          h +=
            '<div class="cmd-group"><div class="cmd-group-label"><i class="fas fa-flag me-1"></i>Reports</div>';
          data.reports.forEach(function (r) {
            h +=
              '<div class="cmd-item" data-type="report" data-id="' +
              r.id +
              '" data-post-id="' +
              r.post_id +
              '"><span class="cmd-type">Report</span><strong>#' +
              r.id +
              ' - ' +
              esc(r.reason) +
              '</strong><span class="text-muted small"> by ' +
      ...
      .catch(function () {
        byId('cmdResults').innerHTML = '<div class="cmd-empty">Search failed.</div>';
      });
  }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (_cmdBuffer === 'gu') {
      _cmdBuffer = '';
      switchTab('users');
    } else if (_cmdBuffer === 'gp') {
      _cmdBuffer = '';
      switchTab('posts');
    } else if (_cmdBuffer === 'gr') {
      _cmdBuffer = '';
      switchTab('reports');
    } else if (_cmdBuffer === 'ga') {
      _cmdBuffer = '';
      switchTab('appeals');
    } else if (_cmdBuffer === 'gl') {
      _cmdBuffer = '';
      switchTab('audit');
    }
  });
  ...
  function openUserPanel(userId) {
    ...
    setBreadcrumb([
      ...
      { label: 'User #' + userId, action: null },
    ]);
    authFetch('/auth/admin/users/' + userId + '/activity')
      .then(function (data) {
        var h =
          '<h3>User Activity <small class="text-muted" style="font-weight:400;font-size:0.85rem;">#ID: ' +
          userId +
          '</small></h3>';
        // Posts
        h +=
          '<div class="side-panel-section"><h6><i class="fas fa-newspaper me-1"></i>Posts (' +
          (data.posts || []).length +
          ')</h6>';
        if (!data.posts || data.posts.length === 0)
          h += '<div class="text-muted small">No posts.</div>';
        else {
          data.posts.forEach(function (p) {
            h +=
              '<div class="sp-item"><span>' +
              esc(p.title || 'Untitled') +
        ...
        // Reports against
        h +=
          '<div class="side-panel-section"><h6><i class="fas fa-flag me-1"></i>Reports Against (' +
          (data.reports_against || []).length +
          ')</h6>';
        if (!data.reports_against || data.reports_against.length === 0)
          h += '<div class="text-muted small">No reports.</div>';
        else {
          data.reports_against.forEach(function (r) {
            h +=
              '<div class="sp-item"><span>' +
              esc(r.reason) +
        ...
        // Bans
        h +=
          '<div class="side-panel-section"><h6><i class="fas fa-ban me-1"></i>Ban History (' +
          (data.bans || []).length +
          ')</h6>';
        if (!data.bans || data.bans.length === 0)
          h += '<div class="text-muted small">No bans.</div>';
        else {
          data.bans.forEach(function (b) {
            var d = b.suspended_until ? new Date(b.suspended_until) : null;
            var isPerm = d && d.getFullYear() >= 2999;
            h +=
              '<div class="sp-item"><span>' +
              esc(b.banned_reason || '-') +
              '</span><span class="badge ' +
              (isPerm ? 'badge-danger' : 'badge-warning') +
              '">' +
              (isPerm ? 'Permanent' : d ? d.toLocaleDateString() : 'Unknown') +
              '</span></div>';
          });
        }
        h += '</div>';
        // Appeals
        h +=
          '<div class="side-panel-section"><h6><i class="fas fa-gavel me-1"></i>Appeals (' +
          (data.appeals || []).length +
          ')</h6>';
        if (!data.appeals || data.appeals.length === 0)
          h += '<div class="text-muted small">No appeals.</div>';
        else {
      ...
      .catch(function () {
        byId('spContent').innerHTML =
          '<div class="text-center py-4 text-danger">Failed to load user data.</div>';
      });
  }
  ...
  function updatePriorityStrip(stats, reports, appeals) {
    ...
    if (reports) {
      reports.forEach(function (r) {
        if (!r.dismissed) pendingReports++;
      });
    }
    if (appeals) {
      appeals.forEach(function (a) {
        if (a.status === 'pending') pendingAppeals++;
      });
    }
    if (pendingReports === 0 && pendingAppeals === 0) {
      el.classList.add('hidden');
      return;
    }
    el.classList.remove('hidden');
    var h =
      '<i class="fas fa-exclamation-triangle text-danger me-1"></i><strong>Needs Attention:</strong>';
    if (pendingReports > 0)
      h +=
        '<span class="strip-item"><span class="badge bg-danger">' +
        pendingReports +
        ' pending report' +
        (pendingReports > 1 ? 's' : '') +
        '</span><button class="strip-action" data-tab="reports">Review</button></span>';
    if (pendingAppeals > 0)
      h +=
        '<span class="strip-item"><span class="badge bg-warning text-dark">' +
        pendingAppeals +
        ' pending appeal' +
        (pendingAppeals > 1 ? 's' : '') +
        '</span><button class="strip-action" data-tab="appeals">Review</button></span>';
    el.innerHTML = h;
    el.querySelectorAll('.strip-action').forEach(function (btn) {
  ...
  function updateBulkBar() {
    ...
    var countEl = byId('bulkUserCount'),
      bar = byId('bulkUserBar');
    if (ids.length === 0) {
  ...
  byId('bulkUserClear').addEventListener('click', function () {
    _selectedUserIds = {};
    updateBulkBar();
    renderUsers(_lastUsers || []);
  });
  byId('bulkUserDelete').addEventListener('click', function () {
    ...
    openConfirm(
      ...
      function () {
        var done = 0;
        ids.forEach(function (id) {
          authFetch('/auth/admin/users/' + id, { method: 'DELETE' })
            .then(function () {
              done++;
              if (done === ids.length) {
                _selectedUserIds = {};
  ...
  function renderPresetChips(containerId, presets, activeKey, onClick) {
    ...
    presets.forEach(function (p, i) {
      ...
      h +=
        ...
        '">' +
        esc(p.label) +
        '<span class="chip-remove" data-key="' +
        key +
        '">&times;</span></span>';
    });
    el.innerHTML = h;
    el.querySelectorAll('.filter-chip:not(.chip-remove)').forEach(function (chip) {
      chip.addEventListener('click', function (e) {
        ...
        if (typeof onClick === 'function') onClick(chip.dataset.key);
      });
    });
    el.querySelectorAll('.chip-remove').forEach(function (rm) {
      rm.addEventListener('click', function (e) {
        e.stopPropagation();
        var key = rm.dataset.key;
        var idx = parseInt(key, 10);
        if (!isNaN(idx) && _filterPresets.users) {
          _filterPresets.users.splice(idx, 1);
  ...
  function renderUsers(users) {
    ...
    var tbody = byId('adminUsersBody'),
      count = byId('userCount');
    if (!tbody) return;
    tbody.innerHTML = '';
    count.textContent = users.length + ' user' + (users.length !== 1 ? 's' : '');
    if (users.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8">' + emptyState('users', 'No users found.') + '</td></tr>';
      return;
    }
    users.forEach(function (u) {
      ...
      row.innerHTML =
        '<td><input type="checkbox" class="user-cb" data-id="' +
        u.id +
        '"' +
        (checked ? ' checked' : '') +
        '></td>' +
        ...
        u.id +
        '</td>' +
        '<td><a href="#" class="user-name-link text-decoration-none fw-semibold" data-id="' +
        u.id +
        ...
        '<td>' +
        (isSuspended
          ? '<span class="badge badge-danger">' + susLabel + '</span>'
          : '<span class="badge badge-ok" style="background:#e6f9ee;color:var(--success);">Active</span>') +
        '</td>' +
        '<td style="white-space:nowrap;">' +
        '<button class="btn-action btn-edit-sm me-1 view-user-btn" data-id="' +
        u.id +
        '" title="View activity"><i class="fas fa-eye"></i></button>' +
    ...
    if (selectAll) {
      selectAll.checked =
        ...
        users.every(function (u) {
          return _selectedUserIds[u.id];
        });
      selectAll.addEventListener('change', function () {
        users.forEach(function (u) {
          if (selectAll.checked) _selectedUserIds[u.id] = true;
          else delete _selectedUserIds[u.id];
        });
    ...
    tbody.querySelectorAll('.user-cb').forEach(function (cb) {
      cb.addEventListener('change', function () {
        if (cb.checked) _selectedUserIds[cb.dataset.id] = true;
        else delete _selectedUserIds[cb.dataset.id];
        updateBulkBar();
      });
    });
    bindUserActions();
  ...
  function renderBannedUsers(users) {
    ...
    if (users.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8">' + emptyState('check-circle', 'No banned users.') + '</td></tr>';
      return;
    }
    users.forEach(function (u) {
      ...
      row.innerHTML =
        ...
        '</span></td>' +
        '<td><button class="btn-action btn-success-sm unsuspend-user-btn" data-user-id="' +
        u.id +
        '"><i class="fas fa-unlock"></i> Revert</button></td>';
      tbody.appendChild(row);
    });
    ...
    tbody.querySelectorAll('.user-name-link').forEach(function (link) {
      link.addEventListener('click', function (e) {
        e.preventDefault();
        openUserPanel(parseInt(link.dataset.id, 10));
      });
    });
  }

  function renderStats(stats, trends) {
    var el = byId('adminStats');
    if (!el) return;
    var s = stats || {},
      t = trends || {};
    ...
    el.innerHTML =
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num" style="color:var(--accent);">' +
      (s.users || 0) +
      '</div>' +
      trendHtml(t.users_7d) +
      '<div class="label">Total Users</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num" style="color:var(--success);">' +
      (s.posts || 0) +
      '</div>' +
      trendHtml(t.posts_7d) +
      '<div class="label">Total Posts</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num" style="color:var(--warning);">' +
      (s.reports || 0) +
      '</div>' +
      trendHtml(t.reports_7d) +
      '<div class="label">Total Reports</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num" style="color:var(--danger);">' +
      (s.suspended || 0) +
      '</div><div class="label">Suspended Users</div></div></div>';
  }

  function renderReports(reports, showResolved) {
    var tbody = byId('reportsBody'),
      count = byId('reportCount');
    if (!tbody) return;
    ...
    if (reports.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8">' +
        emptyState('flag', 'All caught up — no reports to review.') +
        '</td></tr>';
      return;
    }
    reports.forEach(function (r) {
      ...
      if (!isDismissed) {
        if (r.reason === 'Harassment' || r.reason === 'Inappropriate Content')
          severityClass = 'row-ban';
  ...
  function renderPosts(posts) {
    var tbody = byId('adminPostsBody'),
      count = byId('postCount');
    if (!tbody) return;
    tbody.innerHTML = '';
    count.textContent = posts.length + ' post' + (posts.length !== 1 ? 's' : '');
    if (posts.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6">' + emptyState('newspaper', 'No posts found.') + '</td></tr>';
      return;
    }
  ...
  function renderAppeals(appeals) {
    var tbody = byId('appealsBody'),
      count = byId('appealCount');
    if (!tbody) return;
    ...
    tbody.innerHTML = '';
    count.textContent = appeals.length + ' appeal' + (appeals.length !== 1 ? 's' : '');
    if (appeals.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7">' + emptyState('gavel', 'No appeals to review.') + '</td></tr>';
      return;
    }
    appeals.forEach(function (a) {
      ...
      var statusBadge =
        a.status === 'pending'
          ...
          : a.status === 'approved'
            ...
            : 'badge-dismissed';
      var row = document.createElement('tr');
      if (a.status === 'pending') row.className = 'row-warning';
      row.innerHTML =
        '<td>' +
        a.id +
        '</td>' +
        ...
        '</td>' +
        '<td><span class="badge ' +
        (isPerm ? 'badge-danger' : 'badge-info') +
        '">' +
        durStr +
        '</span></td>' +
        ...
        (a.status === 'pending'
          ? '<button class="btn-action btn-success-sm me-1 approve-appeal-btn" data-appeal-id="' +
            a.id +
            '"><i class="fas fa-check"></i> Revert</button>' +
            '<button class="btn-action btn-edit-sm dismiss-appeal-btn" data-appeal-id="' +
            a.id +
            '"><i class="fas fa-times"></i> Ignore</button>'
    ...
    document.querySelectorAll('.dismiss-appeal-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm(
          'Dismiss Appeal',
          'Mark this appeal as dismissed? The ban will remain.',
          function () {
  ...
  function renderAuditLog(log) {
    var tbody = byId('auditBody');
    if (!tbody) return;
    tbody.innerHTML = '';
    if (!log || log.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5">' + emptyState('history', 'No audit log entries.') + '</td></tr>';
      return;
    }
  ...
  function bindUserActions() {
    ...
    document.querySelectorAll('.role-select').forEach(function (sel) {
      sel.addEventListener('change', function () {
        var newRole = sel.value,
          userName = sel.dataset.userName,
          currentRole = sel.dataset.currentRole;
        var desc =
          'Change <strong>' + userName + '</strong> from ' + currentRole + ' to ' + newRole + '.';
        openConfirm(
          'Change Role',
          desc,
          function () {
    ...
    document.querySelectorAll('.view-user-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openUserPanel(parseInt(btn.dataset.id, 10));
      });
    });
  ...
  function loadAdminPanel() {
    ...
    if (_showBannedOnly) {
      authFetch('/auth/admin/banned-users')
        ...
        .catch(function (err) {
          hideEl('adminLoading');
          showEl('adminError');
          byId('adminError').textContent = err.message;
        });
      return;
    }
    // Apply filters
    ...
    if (searchTerm) {
      url = '/auth/admin/search-users?q=' + encodeURIComponent(searchTerm);
    }
    authFetch(url)
      .then(function (data) {
        hideEl('adminLoading');
  ...
  function loadPosts(search, category, dateFilter) {
    ...
    authFetch('/posts/admin/all' + qs)
      ...
      .catch(function (err) {
        hideSkeleton('postsLoading');
        showEl('postsError');
        byId('postsError').textContent = err.message;
      });
  }

  function loadReports(includeResolved) {
    showSkeleton('reportsLoading', 5, 6);
    hideEl('reportsError');
    hideEl('reportsPanel');
    authFetch('/posts/reports' + (includeResolved ? '?includeDismissed=true' : ''))
      .then(function (data) {
        hideSkeleton('reportsLoading');
        showEl('reportsPanel');
        ...
        authFetch('/auth/admin/appeals')
          .then(function (aData) {
            updatePriorityStrip(null, reports, aData.appeals);
          })
          .catch(function () {});
      })
      .catch(function (err) {
        hideSkeleton('reportsLoading');
        showEl('reportsError');
        byId('reportsError').textContent = err.message;
      });
  }

  function loadAppeals() {
    showSkeleton('appealsLoading', 4, 5);
    hideEl('appealsError');
    hideEl('appealsPanel');
    authFetch('/auth/admin/appeals')
      .then(function (data) {
        hideSkeleton('appealsLoading');
        showEl('appealsPanel');
      ...
      .catch(function (err) {
        hideSkeleton('appealsLoading');
        showEl('appealsError');
        byId('appealsError').textContent = err.message;
      });
  }

  function loadAuditLog() {
    showSkeleton('auditLoading', 5, 4);
    hideEl('auditError');
    hideEl('auditPanel');
    authFetch('/auth/admin/audit-log')
      .then(function (data) {
        hideSkeleton('auditLoading');
        showEl('auditPanel');
      ...
      .catch(function (err) {
        hideSkeleton('auditLoading');
        showEl('auditError');
        byId('auditError').textContent = err.message;
      });
  }

  // ---- Filter handlers ----
  byId('userSearch').addEventListener('input', function () {
  ...
  byId('presetCancel').addEventListener('click', function () {
    hideEl('presetModal');
  });
  ...
  var postSearch = byId('postSearch'),
    filterCategory = byId('filterCategory'),
    filterDate = byId('filterDate');
  ...
  byId('showResolvedReportsBtn').addEventListener('click', function () {
    ...
    this.innerHTML = showResolved
      ? '<i class="fas fa-flag me-1"></i>Active Reports'
