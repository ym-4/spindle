(function () {
  ...
  document.getElementById('confirmOk').addEventListener('click', function () {
    if (_confirmStep2) {
      ...
      if (val !== _confirmStep2) { alert('Type "' + _confirmStep2 + '" to confirm.'); return; }
    }
    hideEl('confirmModal');
  ...
  document.getElementById('banReasonSelect').addEventListener('change', function () {
    ...
    if (this.value === 'Other') { txt.classList.remove('hidden'); } else { txt.classList.add('hidden'); }
  });
  document.getElementById('banCancel').addEventListener('click', function () { hideEl('banModal'); _pendingBanUserId = null; });
  document.getElementById('banConfirm').addEventListener('click', function () {
    ...
    if (reason === 'Other') {
      ...
      if (!custom) { alert('Please enter a reason.'); return; }
      reason = custom;
    }
    hideEl('banModal');
  ...
  function renderUsers(users) {
    ...
    users.forEach(function (u) {
      ...
      if (isSuspended) {
        ...
        suspendLabel = until.getFullYear() >= 2999 ? 'Permanent Ban' : 'Banned until ' + until.toLocaleDateString();
      }
      var row = document.createElement('tr');
      row.innerHTML =
        '<td>' + u.id + '</td>' +
        '<td><strong>' + esc(u.display_name || u.name) + '</strong></td>' +
        '<td>' + esc(u.email) + '</td>' +
        '<td>' +
          '<select class="form-select form-select-sm role-select" data-user-id="' + u.id + '" data-user-name="' + esc(u.display_name || u.name) + '" data-current-role="' + u.role + '" style="width:auto;display:inline-block;">' +
            '<option value="user"' + (u.role === 'user' ? ' selected' : '') + '>User</option>' +
            '<option value="admin"' + (u.role === 'admin' ? ' selected' : '') + '>Admin</option>' +
          '</select>' +
        '</td>' +
        '<td>' + (isSuspended ? '<span class="badge bg-danger">' + suspendLabel + '</span>' : '<span class="badge bg-success">Active</span>') + '</td>' +
        '<td style="white-space:nowrap;">' +
          '<button class="btn-action btn-danger-sm me-1 delete-user-btn" data-user-id="' + u.id + '" data-user-name="' + esc(u.display_name || u.name) + '" title="Delete user"><i class="fas fa-trash-alt"></i></button>' +
  ...
  function renderBannedUsers(users) {
    ...
    if (users.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-4">No banned users.</td></tr>';
      return;
    }
    users.forEach(function (u) {
      var until = new Date(u.suspended_until);
      ...
      row.innerHTML =
        ...
        '<td><span class="badge bg-danger">' + (isPerm ? 'Permanent' : 'Until ' + until.toLocaleDateString()) + '</span></td>' +
        '<td style="white-space:nowrap;">' +
          '<button class="btn-action btn-success-sm unsuspend-user-btn" data-user-id="' + u.id + '" title="Unsuspend"><i class="fas fa-unlock"></i> Revert</button>' +
        '</td>';
      tbody.appendChild(row);
    });
    document.querySelectorAll('.unsuspend-user-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm('Unsuspend User', 'Lift the ban on user ID <strong>' + btn.dataset.userId + '</strong>?', function () {
          authFetch('/auth/admin/unsuspend', { method: 'POST', body: JSON.stringify({ user_id: btn.dataset.userId }) })
            .then(function () { loadAdminPanel(); loadStats(); })
  ...
  function renderStats(stats, trends) {
    ...
    var trendHtml = function (val, label) {
      ...
      return '<div class="trend ' + cls + '"><i class="fas fa-' + (parseInt(val) >= 0 ? 'arrow-up' : 'arrow-down') + ' me-1"></i>' + val + ' this week</div>';
    };
    el.innerHTML =
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num text-primary">' + (s.users || 0) + '</div>' + trendHtml(t.users_7d, 'users') + '<div class="label">Total Users</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num text-success">' + (s.posts || 0) + '</div>' + trendHtml(t.posts_7d, 'posts') + '<div class="label">Total Posts</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num text-warning">' + (s.reports || 0) + '</div>' + trendHtml(t.reports_7d, 'reports') + '<div class="label">Total Reports</div></div></div>' +
      '<div class="col-md-3 col-6"><div class="stat-card"><div class="num text-danger">' + (s.suspended || 0) + '</div><div class="label">Suspended Users</div></div></div>';
  }
  ...
  function renderReports(reports, showResolved) {
    ...
    if (reports.length === 0) {
      tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">No reports.</td></tr>';
      return;
    }
    reports.forEach(function (r) {
      ...
      row.innerHTML =
        ...
        '<td><span class="badge bg-warning text-dark">' + esc(r.reason) + '</span></td>' +
        '<td style="max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + esc(r.description || '') + '">' + esc(r.description || '-') + '</td>' +
        ...
        '<td style="white-space:nowrap;">' + (isDismissed ?
    ...
    document.querySelectorAll('.delete-post-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm('Delete Post', 'Delete post #' + btn.dataset.postId + '? This cannot be undone.', function () {
          authFetch('/posts/' + btn.dataset.postId, { method: 'DELETE' })
            .then(function () { loadReports(); loadStats(); })
            .catch(function (err) { alert(err.message); });
  ...
  function renderAppeals(appeals) {
    ...
    appeals.forEach(function (a) {
      ...
      row.innerHTML =
        ...
        '<td style="white-space:nowrap;">' +
          '<button class="btn-action btn-success-sm me-1 approve-appeal-btn" data-appeal-id="' + a.id + '" title="Approve & unsuspend"><i class="fas fa-check"></i> Revert Ban</button>' +
          '<button class="btn-action btn-edit-sm dismiss-appeal-btn" data-appeal-id="' + a.id + '" title="Dismiss"><i class="fas fa-times"></i> Ignore</button>' +
        '</td>';
      tbody.appendChild(row);
    });
    document.querySelectorAll('.approve-appeal-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm('Approve Appeal', 'Unsuspend the user and mark the appeal as approved?', function () {
          authFetch('/auth/admin/appeals/' + btn.dataset.appealId + '/approve', { method: 'POST' })
            .then(function () { loadAppeals(); loadStats(); })
            .catch(function (err) { alert(err.message); });
        });
      });
    });
    document.querySelectorAll('.dismiss-appeal-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm('Dismiss Appeal', 'Mark this appeal as dismissed? The ban will remain in effect.', function () {
          authFetch('/auth/admin/appeals/' + btn.dataset.appealId + '/dismiss', { method: 'POST' })
  ...
  function renderAuditLog(log) {
    ...
    tbody.innerHTML = '';
    if (!log || log.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted py-4">No audit log entries.</td></tr>';
      return;
    }
  ...
  function bindUserActions() {
    document.querySelectorAll('.delete-user-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        openConfirm('Delete User', 'Delete user <strong>' + btn.dataset.userName + '</strong> (ID: ' + btn.dataset.userId + ')? This cannot be undone.', function () {
    ...
    document.querySelectorAll('.role-select').forEach(function (sel) {
      sel.addEventListener('change', function () {
        ...
        var desc = 'You are about to promote <strong>' + userName + '</strong> from ' + currentRole + ' to ' + newRole + '. ' +
          (newRole === 'admin' ? 'Admins can manage users, posts, and reports, including banning other users. This can be reversed later.' : '');
  ...
  function loadAdminPanel() {
    ...
    if (_showBannedOnly) {
      authFetch('/auth/admin/banned-users').then(function (data) {
        hideEl('adminLoading');
        showEl('adminPanel');
  ...
  function loadStats() {
    authFetch('/auth/admin/stats').then(function (data) {
      authFetch('/auth/admin/trend-stats').then(function (trends) {
        renderStats(data, trends);
      }).catch(function () { renderStats(data); });
    }).catch(function () {});
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function reloadPosts() {
      loadPosts(postSearch.value.trim() || undefined, filterCategory.value || undefined, filterDate.value || undefined);
    }
    var searchTimer;
    ...
    userSearch.addEventListener('input', function () {
      ...
      userSearchTimer = setTimeout(function () {
        ...
        if (!term) { _showBannedOnly = false; loadAdminPanel(); return; }
        authFetch('/auth/admin/search-users?q=' + encodeURIComponent(term)).then(function (data) {
