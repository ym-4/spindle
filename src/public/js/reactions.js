      ...
      (status, data) => {
        if (status === 200 && Array.isArray(data)) {
          ...
          data.forEach((reaction) => {
            userReactions.set(parseInt(reaction.post_id), {
              id: reaction.id,
...
function initReactionButtons(postId, likeBtn, dislikeBtn) {
  ...
  updateReactionUI(likeBtn, dislikeBtn, reaction ? reaction.reaction_type : null);
}
...
function setupReactionEvents(postId, likeBtn, dislikeBtn) {
  if (likeBtn) {
    likeBtn.addEventListener('click', (e) => {
      ...
      handleReaction(postId, 'like', likeBtn, dislikeBtn);
    });
  }
  ...
  if (dislikeBtn) {
    dislikeBtn.addEventListener('click', (e) => {
      ...
      handleReaction(postId, 'dislike', likeBtn, dislikeBtn);
    });
  }
}
...
function handleReaction(postId, clickedReaction, likeBtn, dislikeBtn) {
  ...
  if (!existingReaction) {
    fetchMethod(
      ...
      (status, data) => {
        if (status === 201) {
          userReactions.set(parseInt(postId), {
            id: data.id,
            reaction_type: clickedReaction,
          });

          updateReactionUI(likeBtn, dislikeBtn, clickedReaction);

          updateReactionCounts(likeBtn, dislikeBtn, null, clickedReaction);
        }
      },
    ...
    fetchMethod(
      ...
      (status) => {
        if (status === 200) {
          ...
          updateReactionUI(likeBtn, dislikeBtn, null);

          updateReactionCounts(likeBtn, dislikeBtn, clickedReaction, null);
        }
      },
  ...
  fetchMethod(
    ...
    (status, data) => {
      if (status === 200) {
        ...
        updateReactionCounts(likeBtn, dislikeBtn, oldReaction, clickedReaction);
      }
    },
...
function updateReactionCounts(likeBtn, dislikeBtn, oldReaction, newReaction) {
