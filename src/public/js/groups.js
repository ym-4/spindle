...
if (currentURL == `${getCurrentUrl}/groups_page.html`) {
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('school-img').src =
      `./images/Groups_${currentSchoolObj.code}_Building.png`;
    ...
    try {
      ...
      if (school == 'userGroups') {
        ...
        document.getElementById('add-btn').addEventListener('click', () => handleAddButton(school));
...
async function handleCreateButton() {
  ...
  try {
    ...
    // Hide modal
    modal.hide();
    displayToast('success', `Created Group: You have successfully created ${data.name}`);
    ...
    if (err.type === 'conflict') {
      ...
      displayToast('error', `Create Group Failed: Group name already exists`);
...
async function handleLeaveButton() {
  ...
  try {
    ...
    displayToast('success', `Left Group: You have left ${currGroup.name}`);
    ...
    if (err.type == 'conflict') {
      ...
      modal.hide();
      displayToast('error', 'Leave Group Failed: You cannot leave as its creator');
    } else if (err.type == 'not found') {
      const modalElement = document.getElementById('join-group-modal');
      const modal = bootstrap.Modal.getInstance(modalElement);

      modal.hide();
      displayToast('error', 'Leave Group Failed: You are not a member');
    } else if (err.type == 'bad request') {
...
async function handleJoinButton() {
  ...
  try {
    ...
    if (err.type == 'conflict') {
      ...
      modal.hide();
      displayToast('error', 'Join group Failed: You are already a member');
    } else if (err.type == 'not found') {
      const modalElement = document.getElementById('join-group-modal');
      const modal = bootstrap.Modal.getInstance(modalElement);
...
function displayGroupInfo(groupId) {
  ...
  const currGroup = groups.find((group) => {
...
function fetchGroupsBySchool(school) {
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/school/${school.toUpperCase()}`;
...
function fetchJoinedGroups() {
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/joined_groups`;
...
async function fetchGroupJoinRequestsByUser() {
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/join-requests/user`;
...
function createGroup(data) {
  ...
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/create/${user_id}`;
    ...
    const callback = (responseStatus, responseData) => {
      ...
      if (responseStatus == 201) {
        ...
        createGeneralChannel(responseData.id);
        resolve(responseData);
...
function createMember(data) {
  ...
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/join/${requestData.group_id}`;
    ...
    fetchMethod(url, callback, 'POST', requestData, token);
  });
}

function createGeneralChannel(groupId) {
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/messages/send/${user_id}`;

    const data = {
      group_id: groupId,
      channel_name: 'general',
      message: 'General channel created',
    };

    const callback = (responseStatus, responseData) => {
      console.log('createGeneralChannel', responseData);
...
function deleteMember(data) {
  ...
  return new Promise((resolve, reject) => {
    const url = `${getCurrentUrl}/groups/leave/${requestData.group_id}`;
    ...
    fetchMethod(url, callback, 'DELETE', requestData, token);
  });
}
