...
function renderPost(post) {
  ...
  const { timeStr, wasEdited, editedStr } =
  ...
  document.getElementById('postDetailContainer').innerHTML = `
    <div class="post-card" data-post-id="${post.id}" style="cursor: default;">
      <div class="post-header">
        ...
        <div class="post-author">
          ...
          <div class="post-timestamp">
            ...
            ${
              wasEdited
                ? `<span class="post-edited-tag text-muted">&nbsp;·&nbsp;edited on ${editedStr}</span>`
                : ''
            }
          </div>
        </div>
      ...
      ${
        post.title
          ? `<div class="fw-bold mt-2 mb-1" style="font-size:1.05rem;">
...
function loadComments(postId) {
  ...
  fetchMethod(
    ...
    (status, data) => {
      ...
      console.log("COMMENTS RESPONSE:", status, data);
      ...
      if (status === 200 && comments.length > 0) {
        ...
        document.getElementById('totalCommentsLabel').textContent =
          `(${comments.length})`;
...
function setupCommentSubmit(postId) {
  document.getElementById('submitCommentBtn').addEventListener('click', () => {
    ...
    fetchMethod(
      `${COMMENTS_BASE}/${postId}`,
      (status, data) => {
        console.log("POST COMMENT RESPONSE:", status, data);
...
function appendCommentToDOM(comment) {
  ...
  const { timeStr } =
    formatTimestamp(comment.created_at, null);
  ...
  const isOwner =
    loggedInUserId &&
    parseInt(comment.user_id) === loggedInUserId;
        ...
        <div class="comment-content">
          <div class="d-flex align-items-start justify-content-between">
            ...
            <div class="dropdown ms-2">
              <button
                class="btn btn-sm p-0 px-1 comment-menu-btn"
  ...
  if (isOwner) {
    const deleteBtn =
      el.querySelector('.delete-comment-btn');

    const menuBtn =
      el.querySelector('.comment-menu-btn');
    ...
    (status, data) => {
      ...
      if (status === 200) {
        ...
        setTimeout(() => {
          ...
          const countEl =
            document.getElementById('commentCountBtn');
...
function formatTimestamp(createdAt, updatedAt) {
  ...
  const wasEdited =
    updated && Math.abs(updated - created) > 5000;
  ...
  if (diffMins < 1)
    ...
    timeStr =
      `Yesterday at ${created.toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit'
      })}`;
    ...
    timeStr = created.toLocaleDateString([], {
      day: 'numeric',
      month: 'short',
      year: 'numeric'
    });
